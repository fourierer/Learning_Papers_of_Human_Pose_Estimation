...
**再分析SimpleBaseline和HRNet中的Data Transformation和Encoding and Deconding：**
...
现在有一个网络的预测的关键点在图像输出空间的坐标为$^0\hat{k}$，其在图像输入空间坐标为$^i\hat{k}$，在原始图像空间坐标为$^s\hat{k}$，则有以下公式：
$$
^s\hat{k}=
\left[\begin{matrix}
...
^o\hat{k}\tag{4}
$$
公式（4）不知道如何推导来的（这部分推导不会再用到），和前面的公式（2）如何联系？（以下套用公式（2）得到$^s\hat{k}$和$^i\hat{k}$的关系，但是$^i\hat{k}$和$^o\hat{k}$并不仅仅是差个s因子的关系）
$$
^s\hat{k}=
\left[\begin{matrix}
...
注意：此处推导和原文中符号有一点不一样，个人认为这样写更便于理解位移的意义，因为再实际网络预测时，输入图像和输出heatmap的坐标不能再用因子$s$来联系。
